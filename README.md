...
一个面向 Java 技术面试场景的前后端分离题库与后台管理系统。目前已经完成用户、题目、标签、答题记录、错题记录、权限控制以及双 JWT 会话管理，可作为后续接入 AI 模拟面试、智能评分和面试报告的业务底座。

> 当前版本重点建设题库与认证基础能力，尚未接入大模型。项目名称中的 AI Interview Agent 属于后续规划，不将未完成能力描述为已实现功能。
...
- **普通用户**：注册登录、浏览题库、按条件筛选题目、查看题目详情、维护个人资料和修改密码。
- **管理员**：查看数据概览，管理用户、题目和标签，重置用户密码、禁用用户以及吊销用户会话。
- **系统基础能力**：答题记录、错题累计、题目统计、逻辑删除、双令牌认证和 Refresh Token 轮换。
...
> 当前前端主要完成题库浏览与后台管理，答题提交、答题历史和错题本页面仍待补充。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/answer-records` | 创建答题记录 |
| GET | `/api/answer-records/users/{userId}` | 查询用户答题记录 |
| POST | `/api/wrong-questions` | 新增或累加错题 |
| GET | `/api/wrong-questions/users/{userId}` | 查询用户错题 |
| PUT | `/api/wrong-questions/{id}/mastered` | 标记错题为已掌握 |
